<template>
  <!-- 1 -->
  <HeaderNav />
  <!-- 2 -->
  <van-cell-group>
    <van-cell title="报名人" :value="state.data.name" />
    <van-cell title="活动参与人" :value="state.data.attend" />
    <van-cell title="联系电话" :value="state.data.tel" />
    <van-cell title="性别" :value="state.data.gender" />
    <van-cell title="年龄" :value="state.data.age" />
    <van-cell title="居住地址" :value="state.data.location" />
    <van-cell title="报名时间" :value="state.data.time" />
    <van-cell title="打卡时间" :value="state.data.clock" />
    <van-cell>
      <span>审核结果</span>
      <van-radio-group v-model="state.data.checked" direction="horizontal" class="mt-4">
        <van-radio :name="true" class="w-1/2">通过</van-radio>
        <van-radio :name="false">不通过</van-radio>
      </van-radio-group>
    </van-cell>
    <van-cell v-if="state.data.checked === false">
      <span>审核不通过理由*</span>
      <van-field
        v-model="state.data.reason"
        rows="5"
        autosize
        type="textarea"
        placeholder="请输入审核不通过理由..."
        class="bg-gray-100 rounded mt-2"
      />
    </van-cell>
  </van-cell-group>
</template>

<script setup lang="ts">
import { reactive } from 'vue'
import { useRoute } from 'vue-router'
import HeaderNav from '@/components/Common/HeaderNav.vue'
import type { reviewItem } from '@/models'
import { getReviewById } from '@/services'

type stateType = {
  data: reviewItem
}

const route = useRoute()
const state: stateType = reactive({
  data: {} as reviewItem
})

async function init() {
  if (route.query.id) {
    const id = parseInt(route.query.id.toString())
    state.data = await getReviewById(id)
  }
}

init()
</script>