<template>
    <van-nav-bar safe-area-inset-top @click-left="back" class="bg-blue-400">
        ...
        <template #title>
            <div class="text-white text-base">新增活动</div>
        </template>
    </van-nav-bar>
    <div class="flex flex-col space-y-4 p-4">
        ...
        <van-form class="flex flex-col space-y-4">
            ...
            <div class="text-sm">图片上传(可选择多张)</div>
            ...
            <div class="flex space-x-2">
                ...
                <div class="w-40">
                    <!-- <div>
                        <span v-if="state.data.video.name">{{ state.data.video.name }}</span>
                        <span v-else>暂无</span>
                    </div>
                    <div class="text-sm text-gray-400">
                        时长:&nbsp;
                        <span v-if="state.data.video.length">{{ state.data.video.length }}</span>
                        <span v-else>暂无</span>
                    </div>-->
                    <div>xxxxxxx</div>
                    <div class="text-sm text-gray-400">时长:&nbsp;xxxxx</div>
                </div>
